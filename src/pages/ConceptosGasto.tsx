...
export default function ConceptosGasto() {
  ...
  const { searchTerm, setSearchTerm, filteredData } = useTableFilter({
    data: conceptos || [],
    searchFields: ["codigo", "nombre", "descripcion", "categoria", "centro_costo"],
  });
  ...
  const conceptosPorCategoria = filteredData?.reduce((acc, concepto) => {
                    ...
                    <TableRow key={concepto.id}>
                      ...
                      <TableCell>
                        {concepto.limite_maximo
                          ? formatCurrency(concepto.limite_maximo)
                          : "Sin límite"}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => navigate(`/conceptos-gasto/${concepto.id}/documentos`)}
                            title="Gestionar documentos requeridos"
                          >
